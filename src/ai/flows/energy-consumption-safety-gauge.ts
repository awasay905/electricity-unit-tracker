...
const EnergyConsumptionSafetyInputSchema = z.object({
  ...
  historicalUsageData: z
    .string()
    .describe(
      'Historical electricity usage data as a JSON string, including date and usage. For example: [{\
```
    ),
});
...
export const energyConsumptionSafetyGauge = defineFlow(
  {
  ...
  async (input) => {
    const { monthlyGoal, currentUsage, daysElapsed, historicalUsageData } = input;
    
    const prompt = `
        You are an energy consumption analyst. Your task is to assess the electricity usage of a household and provide a safety assessment.
    ...
    const llmResponse = await ai.generate({
      ...
      output: {
          schema: EnergyConsumptionSafetyOutputSchema
      }
    });
